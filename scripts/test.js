...
test( describe => {
	...
	describe( 'default import', () => {
		...
		assert( GUI === _GUI, 'GUI is available as both default and named export' );
	...
	describe( 'destroy', () => {
		...
		const spyKids = function( child ) {
			childCount++;
			spy( child, 'destroy', () => destroyCount++ );
			if ( child.children ) {
				child.children.forEach( spyKids );
			}
		};

		gui.children.forEach( spyKids );

		gui.destroy();

		assert.strictEqual( destroyCount, childCount, 'destroy was called exactly once on each child' );
		assert.strictEqual( gui.children.size, 0, 'children is empty' );
		assert.strictEqual( folder1.children.size, 0, 'children is empty' );
		assert.strictEqual( folder2.children.size, 0, 'children is empty' );
	...
	describe( 'color', () => {
		...
		const obj = { r: 0.47843137254901963, g: 0.14901960784313725, b: 0.6705882352941176 };
